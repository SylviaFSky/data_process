...
pcl::PointCloud<pcl::PointXYZI>::Ptr allPoints;
pcl::PointCloud<pcl::PointXYZI>::Ptr plane_line_cloud_;
pcl::PointCloud<pcl::PointXYZI>::Ptr clouds;
            ...
            for (size_t plane_index2 = plane_index1 + 1;
                ...
                if (plane_list[plane_index1].cloud.size() > 0 &&
                    ...
                    if (line_cloud.size() > 10) {
                        ...
                        for(int pointId = 0; pointId < line_cloud.size(); pointId++){
                            allPoints->points.push_back(line_cloud[pointId]);
                        }
                    }
                    }
                }
...
int main(int argc, char **argv){
    ros::init(argc, argv, "lidarEdge");
    ros::NodeHandle nh;
    string pcdFile = "/home/xwy/WS/fisheye_ws/src/data_process/src/calibration.pcd";
    clouds = pcl::PointCloud<pcl::PointXYZI>::Ptr(new pcl::PointCloud<pcl::PointXYZI>);
    
    pcl::io::loadPCDFile(pcdFile, *clouds);
    vector<VoxelGrid> voxel_list;
    unordered_map<VOXEL_LOC, Voxel *>voxel_map;
    initVoxel(clouds, voxel_size_, voxel_map);
    LiDAREdgeExtraction(voxel_map, ransac_dis_threshold_, plane_size_threshold_,
                      plane_line_cloud_);
    pcl::visualization::CloudViewer viewer("Viewer");
    pcl::PointCloud<pcl::PointXYZI>::Ptr linePoint(new pcl::PointCloud<pcl::PointXYZI>);

    viewer.showCloud(allPoints);
